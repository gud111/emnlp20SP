...
public class LDAOrg implements Serializable {
	...
	public LDAOrg (int numberOfTopics)
	{
		this (numberOfTopics, 50.0, 0.01);
	}

	public LDAOrg (int numberOfTopics, double alphaSum, double beta)
	{
    ...
    for (int di = 0; di < numDocs; di++) {
      try {
        ...
        System.err.println ("LDAOrg and other topic models expect FeatureSequence data, not FeatureVector data.  "
  ...
  private void sampleTopicsForOneDoc (FeatureSequence oneDocTokens, int[] oneDocTopics, // indexed by seq position
	...
	public ArrayList<TreeSet<IDSorter>> getSortedWords () {
		...
		for (int type = 0; type < numTypes; type++) {
			...
			for (int topic=0; topic<topicCounts.length; topic++)
			{
				topicSortedWords.get(topic).add(new IDSorter(type, topicCounts[topic]));				
			}
		}
